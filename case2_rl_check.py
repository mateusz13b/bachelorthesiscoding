...
from case2_rl_train import JointControlPanda, PandaReachWallCase2Task

#python case2_rl_check.py --algo td3 --model path/to/policy.zip --no-render
...
def resolve_model_path(algo_name: str, model_path: str | None, save_dir: str) -> Path:
    """Resolve the path to the saved policy file."""
...
def resolve_log_paths(
    ...
    """Create output paths for EE and joint logs."""
...
def get_arm_joint_angles(robot: JointControlPanda) -> np.ndarray:
    """Return the seven Panda arm joint angles."""
...
def collect_initial_state(robot: JointControlPanda) -> Dict[str, np.ndarray]:
    """Collect the initial robot state for timestep zero logging."""
    return {
...
def save_ee_log(
    ...
    """Save end-effector position and velocity history to CSV."""
...
def save_joint_log(joint_history: np.ndarray, log_path: Path, log_dt: float) -> None:
    """Save joint angle history to CSV."""
...
def save_run_logs(
    ...
    """Save both EE and joint logs for one rollout."""
    ...
    save_ee_log(
        ...
        ee_velocity_history=ee_velocity_history,
        log_path=ee_path,
        log_dt=log_dt,
    )
...
def run_policy_once(
    ...
    """Run one policy rollout and collect EE and joint logs."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Load and run a PPO/SAC/TD3/TQC policy for case 2 with human rendering and CSV logging."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a saved policy (.zip). If omitted, the script uses models/case_2_<algo>_best_policy.zip",
    )
    parser.add_argument("--save-dir", type=str, default="models", help="Directory with saved policy files")
    parser.add_argument(
        "--episode-tag",
        type=str,
        default="episode",
        help="Tag used in output CSV file names",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.02,
        help="Time step used for the time_sec column in saved CSV logs",
    )
    parser.add_argument(
        "--stochastic",
        action="store_true",
        help="Sample stochastic actions instead of using deterministic inference",
    )
    parser.add_argument(
        "--reward-type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1 / 60,
        help="Sleep duration between rendered steps in seconds",
    )
    parser.add_argument(
        "--print-every",
        type=int,
        default=50,
        help="Print rollout progress every N steps. Use 0 to disable progress printing",
    )
    parser.add_argument(
        "--no-render",
        action="store_true",
        help="Disable the human render window",
    )
...
def main() -> None:
    ...
    ee_path, joint_path = save_run_logs(
        ...
        joint_history=rollout["joint_history"],
        ee_history=rollout["ee_history"],
        ee_velocity_history=rollout["ee_velocity_history"],
        log_dt=args.log_dt,
        case_id=CASE_ID,
    )
    ...
    print(f"Number of steps: {rollout['num_steps']}")
    print(f"Success        : {rollout['success']}")
    print(f"Final distance : {rollout['final_distance']:.6f} m")
    print(f"EE log saved to    : {ee_path}")
    print(f"Joint log saved to : {joint_path}")
