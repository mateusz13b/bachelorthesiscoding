...
#python case2_rl_train.py --algo sac
...
class PandaReachWallCase2Task(Reach):
    """Fixed EE goal with a high wall that leaves a lower passage open."""
    ...
    def __init__(self, sim, get_ee_position, reward_type: str = DEFAULT_REWARD_TYPE):
        ...
        z_center = GAP_HEIGHT + WALL_HEIGHT / 2.0
        self.sim.create_box(
            body_name="high_wall",
            half_extents=[WALL_WIDTH / 2.0, WALL_WIDTH_Y / 2.0, WALL_HEIGHT / 2.0],
            mass=0.0,
            position=[WALL_X, 0.0, z_center],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train PPO/SAC/TD3/TQC for Panda case 2 (high wall) with joint-space control."
    )
    parser.add_argument("--algo", required=True, choices=["ppo", "sac", "td3", "tqc"], help="RL algorithm")
    parser.add_argument("--total-timesteps", type=int, default=150_000, help="Training timesteps")
...
def main() -> None:
    ...
    print(f"=== Training {algo_name.upper()} — Case {CASE_ID} high-wall reach, joint control ===")
    print(f"Goal position: {GOAL_POSITION.tolist()}")
    print(f"Wall x-position: {WALL_X}")
    print(f"Lower free-space height: {GAP_HEIGHT}")
